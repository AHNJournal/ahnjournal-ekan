...
class MigrateExecutable extends MigrateExecutableBase {
  ...
  public function import() {
    // Gracefully handle an edge case where the root user might not exist yet.
    if ($this->isCli() && $root_user = $this->getEntityTypeManager()->getStorage('user')->load(1)) {
      $this->getAccountSwitcher()->switchTo($root_user);
    }
